...
user_counts = [500, 600, 700, 800, 900, 1000]
results = []
    ...
    with open("/results/test_stats.csv", "r") as f:
        for line in f:
            if "Aggregated" in line:
                ...
                print(f"  → {parts[8].strip()} req/s, {parts[4].strip()}ms avg latency, {parts[3]} failures")
...
with open("/results/high_load_results.json", "w") as f:
    json.dump(results, f, indent=2)

print("\n" + "="*60)
print("HIGH LOAD TEST SUMMARY")
print("="*60)
for r in results:
    print(f"{r['users']:4d} users: {r['req_per_sec']:6.1f} req/s, "
          f"{r['avg_latency']:5.1f}ms, lag: {r['max_kafka_lag']:3d}, failures: {r['failures']}")
print("="*60)
